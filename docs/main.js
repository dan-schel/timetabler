...
(() => {
  ...
  function drawOutlinedRoundedRect(ctx, x1, y1, x2, y2, radius, strokeStyle) {
    ctx.strokeStyle = strokeStyle;
    ctx.lineWidth = 4;
  ...
  function drawOutlinedGradientRoundedRect(ctx, x1, y1, x2, y2, radius, color1, color2, fillStyle) {
  ...
  function rectGradient(ctx, x1, y1, x2, y2, color1, color2, spread) {
  ...
  // ts/canvas/overflow-visual-block.ts
  ...
  // ts/canvas/primary-visual-block.ts
  var PrimaryVisualBlock = class extends VisualBlock {
    ...
    draw(ctx) {
      ...
      const textColor = this.timetableBlock.online ? this._canvas.css.colorInk80 : colors.on;
      ...
      const nameTextY = blockY1 + (blockHeight - rem(showType ? 2 : 1)) / 2;
      drawText(
        ctx,
  ...
  // ts/canvas/suggestion-visual-block.ts
