...
(() => {
  ...
  var Transition = class {
    ...
    animateTo(value, ignoreIfSameTarget) {
      if (this._target == value && ignoreIfSameTarget != false) {
        return;
      }
  ...
  function drawGradientRoundedRect(ctx, x1, y1, x2, y2, radius) {
    ctx.fillStyle = "black";
    ctx.beginPath();
  ...
  var BlocksRenderer = class {
    ...
    onTimetableUpdate(timetable2) {
      timetable2.choices.forEach((c) => {
        ...
        const existingBlocks = this._blocks.filter((b) => b.timetableClass.equals(timetableClass));
        ...
        const blockCoordinates = timetableBlocks.map((b) => {
          ...
          const firstBlockEnd = endLocation.x == startLocation.x ? endLocation.y : this._gridlines.endHour - this._gridlines.startHour;
          return {
            timetableBlock: b,
            main: {
              x: startLocation.x,
  ...
  var VisualBlock = class {
    constructor(canvas3, gridlines, timetableClass, initialBlock, initialX, initialY1, initialY2, isGhost) {
      this._canvas = canvas3;
      this._gridlines = gridlines;
    ...
    moveTo(newBlock, newX, newY1, newY2) {
  ...
  var CanvasController = class {
    ...
    draw(spontaneous) {
      ...
      if (this._animations.length > 0) {
        ...
        const now = Date.now();
        const delta = (now - ((_a = this._animationLastFrameTime) != null ? _a : now)) / 1e3;
